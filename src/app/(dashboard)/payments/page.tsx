...
import { payments, invoices, agents } from "@/lib/data";
...
export default async function PaymentsPage({
  ...
  const getAgentName = (agentId: string) => agents.find(a => a.id === agentId)?.name || 'ناشناس';
  const getInvoiceNumber = (invoiceId: string) => invoices.find(i => i.id === invoiceId)?.invoiceNumber || '---';
            ...
            <TableBody>
                ...
                {filteredPayments.map((payment) => (
                    <TableRow key={payment.id}>
                        ...
                        <TableCell className="font-code">{new Date(payment.date).toLocaleDateString('fa-IR')}</TableCell>

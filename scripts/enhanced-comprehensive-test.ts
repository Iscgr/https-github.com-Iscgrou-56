...
const SUITE_NAME = 'enhanced-playwright-suite';
...
class EnhancedPlaywrightSuite {
  ...
  private readonly allConsoleLogs: ConsoleLogEntry[] = [];
  ...
  private async setupBrowser(): Promise<void> {
    try {
      this.browser = await chromium.launch({
        headless: true,
        args: SUITE_SETTINGS.launchArgs,
      });
      console.log(chalk.green('✅ مرورگر Playwright راه‌اندازی شد'));
    } catch (error) {
      console.error(chalk.red(`❌ خطا در راه‌اندازی Playwright: ${error}`));
      throw error;
    }
  }
  ...
  private async closeBrowser(): Promise<void> {
    if (this.browser) {
      ...
      console.log(chalk.green('✅ مرورگر Playwright بسته شد'));
    }
  }
  ...
  private async createPage(pageName: string): Promise<{
    ...
    const context = await this.browser.newContext({
      viewport: SUITE_SETTINGS.viewport,
    });
    const page = await context.newPage();
    page.setDefaultTimeout(SUITE_SETTINGS.timeout);
    page.setDefaultNavigationTimeout(SUITE_SETTINGS.timeout);
  ...
  private async executeTest(module: string, testName: string, handler: TestHandler) {
    ...
    const { context, page, collector } = await this.createPage(`${module}-${testName}`);
  ...
  private nextScreenshotPath(label: string) {
    ...
    return this.reporter.resolveScreenshotPath(filename);
  }
  ...
  async testUIComponents(): Promise<void> {
    ...
    await this.executeTest('UI', 'پروفایل نماینده', async (page) => {
      ...
      await page.goto(this.buildUrl(`/agents/${agentId}`), { waitUntil: 'networkidle' });
    ...
    await this.executeTest('UI', 'فرم پرداخت', async (page) => {
      await page.goto(this.buildUrl('/payments'), { waitUntil: 'networkidle' });
  ...
  async testUserScenarios(): Promise<void> {
    ...
    await this.executeTest('UserScenario', 'مشاهده گزارش‌ها', async (page) => {
      await page.goto(this.buildUrl('/reports'), { waitUntil: 'networkidle' });
  ...
  async testHydrationIssues(): Promise<void> {
    ...
    for (const pageInfo of pages) {
      await this.executeTest('Hydration', `Hydration ${pageInfo.name}`, async (page, collector) => {
        await page.goto(this.buildUrl(pageInfo.url), { waitUntil: 'networkidle' });
  ...
  async runAllTests(): Promise<void> {
    try {
      await this.environmentManager.withEnvironment(async (context) => {
        ...
        console.log(chalk.blue('🚀 شروع اجرای تست‌های Playwright...'));
      ...
      console.error(chalk.red(`❌ خطا در اجرای Playwright: ${error}`));
    } finally {
      this.finalizeReport();
    }
  }
...
if (require.main === module) {
  const testSuite = new EnhancedPlaywrightSuite();
  testSuite
    .runAllTests()
    .then(() => {
      console.log(chalk.green('\n✅ اجرای تست‌های Playwright به پایان رسید.'));
    })
    .catch((error) => {
      console.error(chalk.red(`\n❌ خطا در اجرای تست‌های Playwright: ${error}`));
      process.exit(1);
    });
}

export { EnhancedPlaywrightSuite };